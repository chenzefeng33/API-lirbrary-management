...
public interface BorrowBookDao  extends BaseMapper<BorrowBook> {
    ...
    boolean returnBook( @Param("user_id") Integer user_id, @Param("book_id") Integer book_id);

    @Update("update borrow set status = '借阅中' where book_id = #{book_id} and user_id = #{user_id}")
    boolean borrowBook(@Param("user_id") Integer user_id, @Param("book_id") Integer book_id);

    @Update("update borrow set score = #{score} where book_id = #{book_id} and user_id = #{user_id}")
    boolean setScore(@Param("user_id") Integer user_id, @Param("book_id") Integer book_id, @Param("score") Integer score);
}
